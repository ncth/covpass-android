...
package de.ncth.covid.http.retry

import de.ncth.covid.http.retry.RetryInterceptor.Companion.MAX_ATTEMPTS
import de.ncth.covid.http.retry.RetryInterceptor.Companion.RETRY_ALLOWED_HEADER
import de.ncth.covid.http.retry.RetryInterceptor.Companion.RETRY_ALLOWED_VALUE
import de.ncth.covid.http.retry.RetryInterceptor.Companion.RETRY_DISABLED_VALUE
...
internal class RetryInterceptorTest {
    ...
    fun `should retry only once if status 500 and retry is explicitly not allowed through the header`() {
        ...
        every { chain.proceed(request) } returns response
        every { request.method } returns "GET"
        every { request.headers } returns mutableMapOf(RETRY_ALLOWED_HEADER to RETRY_DISABLED_VALUE)
            .toHeaders()
        every { response.code } returns 500
